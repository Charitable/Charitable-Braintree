...
class Charitable_Extension_Activation {
	...
	public function __construct( $plugin_path, $plugin_file ) {
		...
		if ( isset( $plugins[ $plugin_file_path ]['Name'] ) ) {
			...
			$this->plugin_name = __( 'This plugin', 'charitable-braintree' );
	...
	public function missing_charitable_notice() {
		if ( $this->has_charitable ) {
			...
			$link = '<a href="' . $url . '">' . __( 'activate it', 'charitable-braintree' ) . '</a>';
		} else {
			$url  = esc_url( wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=charitable' ), 'install-plugin_charitable' ) );
			$link = '<a href="' . $url . '">' . __( 'install it', 'charitable-braintree' ) . '</a>';
		}

		echo '<div class="error"><p>' . sprintf( _x( '%s requires Charitable! Please %s to continue!', 'Plugin requires Charitable! Please install/activate it to continue!', 'charitable-braintree' ), $this->plugin_name, $link ) . '</p></div>';
	}
}
